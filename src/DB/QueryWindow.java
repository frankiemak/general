...
public class QueryWindow {
	...
	private static void InitTextFields() {
		...
		mDatabaseCB = new JComboBox(new String[] { "jdbc:mysql://localhost:3306/rates" });
	...
	private static void InitButtons() {
		...
		mConnectButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				if (!mUsernameTF.getText().isEmpty()) {
					mDBEngine = new DBEngine("jdbc:mysql://localhost:3306/rates", mUsernameTF.getText(),
							mPasswordTF.getText());
					...
					if (mDBEngine.getConnection() != null) {
						mConnectionL.setText("jdbc:mysql://localhost:3306/rates");
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (mQuery.toLowerCase().startsWith("select")) {
					...
					try {
						...
						mSQLResultsArea.setModel(new DefaultTableModel(
								new String[][] { new String[] { "Row Updated!" } }, new String[] { "" }));
					} catch (SQLException e) {
						JOptionPane.showMessageDialog(mMainPanel, "Could not execute query!");
						// e.printStackTrace();
					}
		...
		mClearResultsButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				mSQLResultsArea
						.setModel(new DefaultTableModel(new String[][] { new String[] { "" } }, new String[] { "" }));
	...
	private static void InitPlacement() {
		...
		mSpringLayout.putConstraint(SpringLayout.WEST, mCommandScrollPane, 10, SpringLayout.HORIZONTAL_CENTER,
				mMainPanel);
		...
		mSpringLayout.putConstraint(SpringLayout.SOUTH, mCommandScrollPane, -10, SpringLayout.NORTH,
				mClearCommandButton);

		mSpringLayout.putConstraint(SpringLayout.EAST, mClearCommandButton, -10, SpringLayout.EAST, mMainPanel);
		mSpringLayout.putConstraint(SpringLayout.SOUTH, mClearCommandButton, -10, SpringLayout.VERTICAL_CENTER,
				mMainPanel);
		...
		mSpringLayout.putConstraint(SpringLayout.NORTH, mResultsScrollPane, 10, SpringLayout.SOUTH,
				mClearResultsButton);
		mSpringLayout.putConstraint(SpringLayout.EAST, mResultsScrollPane, -10, SpringLayout.EAST, mMainPanel);

		mSpringLayout.putConstraint(SpringLayout.NORTH, mClearResultsButton, 10, SpringLayout.VERTICAL_CENTER,
				mMainPanel);
		...
		mSpringLayout.putConstraint(SpringLayout.HORIZONTAL_CENTER, mJSeparator, 0, SpringLayout.HORIZONTAL_CENTER,
				mMainPanel);
		mSpringLayout.putConstraint(SpringLayout.VERTICAL_CENTER, mJSeparator, 0, SpringLayout.VERTICAL_CENTER,
				mMainPanel);
